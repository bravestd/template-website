...
// scooped packages are not supported in bulk lookups
const scoopedPackages = [
  "@gorhom/bottom-sheet",
...
async function getNumOfDownloads() {
  try {
    ...
    for (const scoopedPackage of scoopedPackages) {
      const { data } = await axios.get(`${baseUrl}/${scoopedPackage}`);
      stats[scoopedPackage] = data;
    }
